...
// Profile carries the task sub-agent model/effort resolved for this call.
type Profile struct {
	Model  string
	Effort string
}
...
type Tool struct {
	...
	ParentID string
	FileDiff
	Profile *Profile // ToolDispatch: task sub-agent model/effort
}
...
type Approval struct {
	...
	Tool    string
	Subject string
	// Preview is a human-readable one-line description of what the tool call
	// will do, generated by permission.Preview(). It is shown in the approval
	// popup dialog. The format varies by tool type:
	//   - bash: the full command text
	//   - task: the description or first N characters of the prompt (truncated)
	//   - edit_file/write_file: the file path and a summary of the change
	//   - other tools: a short generic description
	Preview string
...
type Event struct {
	...
	SessionCost     float64    // Usage: cumulative conversation spend in CNY via Pricing.CostInCNY
	SessionCurrency string     // Usage: currency symbol for SessionCost (always "¥" after CostInCNY)
