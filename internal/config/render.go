...
func RenderTOMLForScope(c *Config, scope RenderScope) string {
	...
	if len(c.Permissions.MainAgentAllowed) > 0 {
		...
		b.WriteString("# main_agent_allowed = [\"task\", \"ask\", \"note\", \"audit_finish\", \"read_skill\", \"run_skill\", \"slash_command\", \"memory_get\", \"memory_save\", \"memory_forget\", \"peek-job\", \"cancel-job\", \"steer-job\"]\n")
	}
	b.WriteString("\n")
