...
func newHTTPClient(cfg provider.Config) (*http.Client, error) {
	...
	if !ok && cfg.Extra["proxy_spec"] != nil {
		return nil, fmt.Errorf("anthropic: proxy_spec has unexpected type %T", cfg.Extra["proxy_spec"])
	}
...
func (c *client) buildRequest(req provider.Request) anthRequest {
	...
	for _, m := range provider.SanitizeHistory(req.Messages, req.EffectiveKeepMultimodalTurns()) {
		switch m.Role {
