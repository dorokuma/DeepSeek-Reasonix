...
// expandSingleAsteriskBold promotes *title* (cron templates / Telegram habit) to
// **title** so formatMessage emits MarkdownV2 bold (*…*), not italic (_…_).
func expandSingleAsteriskBold(content string) string {
	const ph = "\x00DOUBLESTAR\x00"
	s := strings.ReplaceAll(content, "**", ph)
	s = regexp.MustCompile(`\*([^*\n]+)\*`).ReplaceAllString(s, "**$1**")
	return strings.ReplaceAll(s, ph, "**")
}

// formatCronMessage formats cron push bodies: single-asterisk titles are bold.
func formatCronMessage(content string) string {
	return formatMessage(expandSingleAsteriskBold(content))
}
