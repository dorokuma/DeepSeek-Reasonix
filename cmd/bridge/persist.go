...
type chatRecord struct {
	...
	HMACKey         string `json:"hmac_key,omitempty"` // AES-256-GCM encrypted (key=sha256(TG_BOT_TOKEN)), base64
	SubagentDisplay     string `json:"subagent_display,omitempty"`
	SubagentReportMode  string `json:"subagent_report_mode,omitempty"`
}
...
func encryptFromPlain(src, dst string) error {
	...
	if len(data) == 0 {
		...
		if err := os.Remove(dst); err != nil && !os.IsNotExist(err) {
			log.Printf("encryptFromPlain: remove %s: %v", dst, err)
		}
		return nil
	}
...
// encryptHMACKey encrypts a 32-byte HMAC key using AES-256-GCM with a key derived
// from the bot token and a random 16-byte salt.  Returns a base64-encoded blob:
//   old format:                              nonce || ciphertext
//   new format (salt prefix, version 2):     salt (16) || nonce || ciphertext
// The salt prevents an attacker who obtains the encrypted blob from brute-forcing
// the bot token directly.
func encryptHMACKey(plainKey []byte, botToken string) (string, error) {
	if len(plainKey) == 0 {
		return "", fmt.Errorf("empty plain key")
	}
	...
	// Derive key: SHA-256(botToken || salt)
	key := sha256.Sum256(append([]byte(botToken), salt...))
	...
	// Prepend salt to output: salt || nonce || ciphertext
...
// decryptHMACKey decrypts a base64-encoded ciphertext produced by encryptHMACKey.
// It tries the new salted format first and falls back to the legacy (unsalted)
// format for backward compatibility.
func decryptHMACKey(cipherB64 string, botToken string) ([]byte, error) {
	if cipherB64 == "" {
	...
	// Try new salted format first (salt = 16 bytes).
	// Determine GCM nonce size after constructing an AES-GCM.
	// Since we only know GCM nonce is 12 bytes for AES-256-GCM, we can
	// derive nonceSize directly.
	const saltLen = 16
	if len(ciphertext) >= saltLen {
		salt := ciphertext[:saltLen]
		rest := ciphertext[saltLen:]
		key := sha256.Sum256(append([]byte(botToken), salt...))
		block, err := aes.NewCipher(key[:])
		if err == nil {
			aesgcm, err := cipher.NewGCM(block)
	...
	// Fallback: legacy unsalted format (SHA-256(botToken)).
	...
	if len(plain) != 32 {
		return nil, fmt.Errorf("decrypted key has wrong length %d", len(plain))
	}
	return plain, nil
}