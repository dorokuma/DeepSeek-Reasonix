...
func (b *Bridge) handleModel(chatID int64, args string) {
	if args == "" {
		...
		b.sendMessage(chatID, fmt.Sprintf("🤖 当前模型: `%s`", model))
		return
	}

	// Switch to new model.
	b.sendMessage(chatID, fmt.Sprintf("🔄 正在切换模型到 `%s`…", args))

	if err := b.sm.SwitchModel(b.ctx, chatID, args); err != nil {
		log.Printf("/model error (chat %d): %v", chatID, err)
		b.sendMessage(chatID, fmt.Sprintf("❌ 模型切换失败: %v", err))
		return
	}

	b.sendMessage(chatID, fmt.Sprintf("✅ 已切换至模型 `%s`", args))
}
