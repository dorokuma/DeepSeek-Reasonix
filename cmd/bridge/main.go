...
func loadConfig() (*Config, error) {
	...
	// 4. Collect secrets for log redaction.
	// Provider keys stay in process env for MCP/boot but must still be redacted in logs.
	// Only Telegram/CF tokens are Unsetenv to shrink /proc environ exposure.
	...
	clearKeys := []string{"TG_BOT_TOKEN", "TG_CRON_BOT_TOKEN", "CF_TOKEN"}
	for _, key := range clearKeys {
		addSecret(os.Getenv(key))
	}
	for _, key := range []string{
		"DEEPSEEK_API_KEY", "JINA_API_KEY", "OPENAI_API_KEY", "ANTHROPIC_API_KEY",
		"MIMO_API_KEY", "OPENCODE_API_KEY", "GEMINI_API_KEY", "GROQ_API_KEY",
	} {
		addSecret(os.Getenv(key))
	}

	// 5. Clear bot/CF env to reduce /proc leak
	for _, key := range clearKeys {
		os.Unsetenv(key)
	}
...
func main() {
	...
	// Signal or /restart → same light shutdown (index+snapshot already in Shutdown).
	...
	case <-restartRequested():
		log.Println("shutdown: restart requested")
	}
