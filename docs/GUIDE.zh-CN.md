...
# subagent_model = "deepseek-pro"     # task 子代理的默认模型
...
核心规则很小：`Shift+Tab` 切换工具审批模式，`Ctrl/Cmd+Y` 只管 YOLO，粘贴继续走系统粘贴快捷键。

### 桌面端 GUI

| 按键或控件 | 作用 | 说明 |
| --- | --- | --- |
| `Shift+Tab` | 切换工具审批模式 | 输入框快捷键。在 Ask/Auto 与 YOLO (bypass) 之间切换。 |
...
| 按键或命令 | 作用 | 说明 |
| --- | --- | --- |
| `Shift+Tab` | 切换工具审批模式 | 在 Ask/Auto 与 YOLO (bypass) 之间切换。 |
...
| YOLO | 跳过普通工具审批；`deny` 和用户 `ask` 问题仍会提示。 |
...
`reasonix chat` 里，内置命令（`/compact`、`/new`、`/clear`、`/rewind`、`/tree`、`/branch`、`/switch`、`/todo`、`/model`、`/mcp`、`/skills`、`/hooks`、`/memory`、`/output-style`、`/sandbox`、`/reasoning-language`、`/help`）在本地执行——`/help` 可列出全部。
