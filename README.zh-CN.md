...
`mcp_<server>__<tool>` 暴露给模型，与 Claude Code 一致；声明 MCP `readOnlyHint: true`
的工具会参与并行调度并命中权限层的只读默认放行。

服务器的 **prompts** 会暴露成 `/mcp_<server>__<prompt>` 斜杠命令（命令后空格分隔参
...
grep）、TOML 配置、交互式 `reasonix setup` 向导、双模型协同（执行器 + 规划器，
...
MCP 客户端——**stdio + Streamable HTTP** 传输、工具（`mcp_server__tool`,支持
